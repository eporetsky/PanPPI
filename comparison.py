...
class Comparison:
    ...
    def calculateJaccardSimilarity(self):
        ...
        for network in self.geneModules.keys():
            num = num + len(self.geneModules[network])
            tmp = [f"{network}:" + s for s in self.geneModules[network].keys()]
            names = names + tmp
        ...
        for network1 in self.geneModules.keys():
            for network2 in self.geneModules.keys():
                if network1 != network2:
                    modules1 = self.geneModules[network1].keys()
                    modules2 = self.geneModules[network2].keys()
                    for module1 in modules1:
                        for module2 in modules2:
                            list1 = self.geneModules[network1][module1]
                            list2 = self.geneModules[network2][module2]
                            jaccard_similarity.loc[
                                f"{network1}:{module1}", f"{network2}:{module2}"] = PyWGCNA.Comparison.jaccard(list1, list2)
                else:
                    modules = self.geneModules[network1].keys()
    ...
    def calculatePvalue(self):
        ...
        genes = []
        for network in self.geneModules.keys():
            for module in self.geneModules[network].keys():
                genes = genes + self.geneModules[network][module]
    ...
    def dfJaccardSimilarity(self, cutoff=0.1, file_name="jaccard_similarity.edges"):
        """
        Plot jaccard similarity matrix as a network
